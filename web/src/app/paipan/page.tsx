...
const WX_ADVICE: Record<string, { colors: string[]; items: string[]; actions: string[]; direction: string }> = {
  "金": { colors: ["白","银","浅灰","米白"], items: ["银色项链或耳钉","白色运动鞋","金属框眼镜","浅灰围巾"], actions: ["整理桌面做一次断舍离","把明天的待办清单写下来","早上7-9点做最重要的决定"], direction: "西北" },
  "木": { colors: ["绿","青","翠绿","浅蓝"], items: ["绿色T恤或衬衫","木质手串","帆布包或棉麻围巾","绿色手机壳"], actions: ["早起锻炼舒展身体","养一盆绿植放在窗台","写下未来三个月的规划"], direction: "东" },
  "水": { colors: ["黑","深蓝","藏青","深灰"], items: ["黑色镜框或墨镜","深蓝色背包","黑色手表","深灰鸭舌帽"], actions: ["多喝水泡一壶茶","冥想静坐10分钟","重要会议或洽谈安排在下午"], direction: "北" },
  "火": { colors: ["红","紫","玫红","橙红"], items: ["红色围巾或丝巾","红绳手链","红色棒球帽","玫红色口红或唇膏"], actions: ["主动联系三个朋友或客户","重要电话放在中午11-13点","穿亮色衣服出门社交"], direction: "南" },
  "土": { colors: ["黄","棕","卡其","米色"], items: ["棕色皮带或手表带","卡其色外套","米色运动鞋","棕色皮质钱包"], actions: ["把手头的事一件一件做好","每工作45分钟站起来休息5分钟","周末去公园散散步踩踩泥土"], direction: "西南" },
};
                  ...
                  <div className="dao-card space-y-4" style={{ borderColor: "rgba(201,169,110,0.35)" }}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div><p className="text-[10px] text-text-tertiary">有利方位</p><p className="text-sm font-medium text-text">{adv.direction}方</p></div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-lg">🎒</span>
                        <div><p className="text-[10px] text-text-tertiary">随身搭配</p><p className="text-xs text-text">{adv.items.slice(0, 2).join(" · ")}</p></div>
                      </div>
                    </div>
                  </div>
                  {/* 卡片 2：理解原理 */}
