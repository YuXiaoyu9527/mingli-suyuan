  ...
  const ScoreBar = ({ label, score, max, detail }: {label:string,score:number,max:number,detail:string}) => (
    <div className="mb-3">
      <div className="flex items-center justify-between mb-1">
        <span className="text-xs text-dao-ink">{label}</span>
        <span className="text-[11px] text-dao-gold">{score}/{max}</span>
      </div>
      <div className="h-1.5 bg-dao-paper-dark rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${score>=max*0.7?"bg-dao-jade":score>=0?"bg-dao-gold":"bg-dao-red"}`}
          style={{width:`${Math.max(0,Math.min(100,(score/max)*100))}%`}}/>
      </div>
      <p className="text-[10px] text-dao-aged-light mt-1">{detail}</p>
    </div>
  );
  ...
  return (
    <div className="flex flex-col min-h-dvh pb-20">
      <header className="px-5 pt-10 pb-4">
        <h1 className="text-2xl text-dao-ink font-[family-name:var(--font-display)] tracking-wider">八字合婚</h1>
        <p className="text-dao-aged text-sm mt-1">年柱根基 · 夫妻宫 · 五行互补 · 十神匹配</p>
      </header>
        ...
        <div className="dao-card space-y-3">
          ...
          <div className="flex gap-2">
            <input type="number" value={m.year} onChange={e=>setM({...m,year:+e.target.value})}
              className="flex-1 bg-dao-paper-dark border border-dao-paper-darker rounded-lg px-2 py-2 text-dao-ink text-xs" placeholder="年"/>
            <input type="number" value={m.month} onChange={e=>setM({...m,month:+e.target.value})}
              className="w-12 bg-dao-paper-dark border border-dao-paper-darker rounded-lg px-2 py-2 text-dao-ink text-xs" placeholder="月"/>
            <input type="number" value={m.day} onChange={e=>setM({...m,day:+e.target.value})}
              className="w-12 bg-dao-paper-dark border border-dao-paper-darker rounded-lg px-2 py-2 text-dao-ink text-xs" placeholder="日"/>
            <input type="number" value={m.hour} onChange={e=>setM({...m,hour:+e.target.value})}
              className="w-12 bg-dao-paper-dark border border-dao-paper-darker rounded-lg px-2 py-2 text-dao-ink text-xs" placeholder="时"/>
          </div>
          <p className="text-xs font-bold text-dao-ink">女方</p>
          <div className="flex gap-2">
          ...
          <button onClick={handleSubmit} disabled={loading}
            className="w-full py-2.5 bg-dao-red text-white rounded-lg text-sm font-medium tap-active flex items-center justify-center gap-1.5">
        ...
        {result && (
          <div className="space-y-4 anim-enter">
            ...
            <div className="text-center py-6 dao-card">
              <p className="text-dao-aged text-xs">{result.male_bazi}</p>
              <p className="text-dao-aged text-xs mt-1">{result.female_bazi}</p>
              <div className="my-4">
                <span className={`text-4xl font-bold ${result.total_score>=60?"text-dao-jade":result.total_score>=40?"text-dao-gold":"text-dao-red"}`}>
                  {result.total_score}
                </span>
                <span className="text-dao-aged ml-2">/ 80</span>
              </div>
              <p className={`text-lg font-bold ${result.total_score>=60?"text-dao-jade":result.total_score>=40?"text-dao-gold":"text-dao-red"}`}>
                {result.level}
              </p>
            </div>
            ...
            <div className="dao-card">
              <h3 className="text-sm font-bold text-dao-ink mb-3">四维分析</h3>
            ...
            {/* 总结 */}
            <div className="dao-card">
              <h3 className="text-sm font-bold text-dao-ink mb-2">综合评语</h3>
              <p className="text-sm text-dao-ink-light leading-relaxed">{result.summary}</p>
            </div>

            <p className="text-center text-[10px] text-dao-aged-light pb-4">
