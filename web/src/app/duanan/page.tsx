  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-dvh pb-20 items-center justify-center gap-3">
        ...
        <p className="text-dao-aged text-sm">加载题库...</p>
        <BottomNav />
      </div>
    );
  }
      ...
      <header className="px-5 pt-10 pb-2">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs text-dao-aged bg-dao-paper-dark px-2.5 py-1 rounded-full tap-active">
            {filterChapter || "全部章节"} ▾
          </button>
        </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          <Swords size={14} className="text-dao-gold" />
          <span className="text-[11px] text-dao-aged-light min-w-[72px]">
            {totalCompleted}/{totalQuestions} 已过
          </span>
          <div className="flex-1 h-1.5 bg-dao-paper-dark rounded-full overflow-hidden">
            <div className="h-full bg-dao-gold rounded-full transition-all duration-500"
              style={{ width: `${chapterProgress}%` }} />
          </div>
          <span className="text-[11px] text-dao-gold font-medium">{chapterProgress}%</span>
        </div>
      </header>
            ...
            <div className="dao-card mb-3">
              <button onClick={() => { setFilterChapter(""); setShowChapters(false); }}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm mb-1
                  ${!filterChapter ? "bg-dao-red/10 text-dao-red font-bold" : "text-dao-ink-light"}`}>
              ...
              {chapters.map((ch) => (
                <button key={ch.name}
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm mb-1
                    ${filterChapter === ch.name ? "bg-dao-red/10 text-dao-red font-bold" : "text-dao-ink-light"}`}>
                  {ch.name}（{ch.count}题）
                </button>
              ))}
            </div>
      ...
      <div className="px-5 flex-1 space-y-4 mt-2">
        ...
        <div className="flex items-center justify-between text-xs text-dao-aged">
          <button onClick={prevQuestion} disabled={currentIdx === 0}
            className={`tap-active ${currentIdx === 0 ? "opacity-30" : "text-dao-indigo"}`}>
            ← 上一题
          </button>
          <span>{currentIdx + 1} / {questions.length}</span>
          <button onClick={nextQuestion} disabled={currentIdx >= questions.length - 1}
            className={`tap-active ${currentIdx >= questions.length - 1 ? "opacity-30" : "text-dao-indigo"}`}>
            下一题 →
          </button>
        </div>
        ...
        {currentQ && progress[currentQ.id] && (
          <div className="text-center">
            <span className="inline-block px-2 py-0.5 bg-dao-jade/10 text-dao-jade text-[10px] rounded-full border border-dao-jade/20">
              ✅ 已完成
            </span>
          </div>
        )}
        ...
        {/* 难度标记 */}
        {currentQ && (
          <div className="flex items-center gap-1 text-[10px] text-dao-aged">
            <span>难度：</span>
            {[1, 2, 3].map((d) => (
              <span key={d} className={d <= currentQ.difficulty ? "text-dao-gold" : "text-dao-paper-darker"}>
                ★
              </span>
            ))}
          </div>
        ...
        <div className="dao-card">
          <div className="flex items-start gap-3 mb-3">
            ...
            <div>
              <h2 className="text-base font-bold text-dao-ink">
                {currentQ?.title}
              </h2>
              <p className="text-[10px] text-dao-aged mt-0.5">{currentQ?.chapter}</p>
            </div>
          </div>
          <div className="text-sm text-dao-ink-light leading-relaxed whitespace-pre-line font-[family-name:var(--font-body)]">
            {currentQ?.story}
          </div>
        </div>
        ...
        {currentQ && (
          <div className="dao-card" style={{ borderColor: "rgba(201, 169, 110, 0.3)" }}>
            <p className="text-sm font-bold text-dao-ink mb-4">
              ❓ {currentQ.question}
            </p>
            ...
            <div className="space-y-2">
              {currentQ.options.map((opt, index) => {
                ...
                let style = "border-dao-paper-darker hover:border-dao-gold/50";
                if (isSelected && isCorrect) style = "border-dao-jade bg-dao-jade/5 ring-1 ring-dao-jade/30";
                else if (isSelected && isCorrect === false) style = "border-dao-red bg-dao-red/5 ring-1 ring-dao-red/30";
                ...
                return (
                  <button key={index}
                    ...
                    <span className={`flex-shrink-0 w-7 h-7 rounded-full flex items-center
                                     ...
                                     ${isSelected
                                       ? isCorrect
                                         ? "bg-dao-jade text-white border-dao-jade"
                                         : "bg-dao-red text-white border-dao-red"
            ...
            <AnimatePresence>
              {selected !== null && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-4">
                  {isCorrect ? (
                    <div className="bg-dao-jade/10 border border-dao-jade/30 rounded-lg p-4">
                      <p className="text-sm font-bold text-dao-jade mb-2">✅ 断案正确！</p>
                      {classicalRef && (
                        <div className="classical-quote text-sm">{classicalRef}</div>
                      )}
                      {mentorLoading && <p className="text-xs text-dao-aged mt-2 flex items-center gap-1">
                        <Loader2 size={12} className="animate-spin" />AI导师点评中...</p>}
                      {mentorHint && <p className="text-sm text-dao-ink-light mt-3 leading-relaxed whitespace-pre-line">{mentorHint}</p>}
                      <button onClick={nextQuestion} disabled={currentIdx >= questions.length - 1}
                        className="mt-3 flex items-center gap-1 text-sm text-dao-indigo font-medium tap-active">
                        <ChevronRight size={16} /> {currentIdx >= questions.length - 1 ? "已是最后一题" : "进入下一案"}
                      </button>
                    </div>
                  ) : (
                    <div className="bg-dao-red/10 border border-dao-red/30 rounded-lg p-4">
                      <p className="text-sm font-bold text-dao-red mb-2">❌ 还需推敲</p>
                      {mentorLoading && <p className="text-xs text-dao-aged flex items-center gap-1">
                        <Loader2 size={12} className="animate-spin" />AI导师提示中...</p>}
                      {mentorHint && <p className="text-sm text-dao-ink-light leading-relaxed whitespace-pre-line">{mentorHint}</p>}
                      <button onClick={resetQuestion} className="mt-2 text-sm text-dao-indigo font-medium tap-active">
                        重新作答
                      </button>
                    </div>
                  )}
